        ...
        <div style="height:aoto; width:100%">
          ...
          <div :class="currentSong.id?'song-list-wrapper haveplayer':'song-list-wrapper'">
            <div class="sequence-play">
              <span class="text" @click.stop="playSongList">
...
export default {
  ...
  activated () {
    this.pageActivity()
    this.$bus.on('singer-detail-acrive', this.pageActivity)
  },
  ...
  methods: {
    ...
    pageActivity (params) {
      ...
      this.inPage = true
      this.$nextTick(() => {
        params = params ? params.params : this.$route.params
        if (params && params.id && `${params.id}` !== `${this.id}`) {
          clearTimeout(this.timer)
          this.songs = []
          this.scrollY = 0
          this.id = ''
        ...
        artistDetail(params.id).then(res => {
          res = res.data
          if (res.code + '' === '200') {
            let list = res.hotSongs ? res.hotSongs : []
